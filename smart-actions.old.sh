...
audio_file_to_text() {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    -f)
      ...
      ;;
    *)
      echo "Error: unknown parameter ($1)! Missing -f parameter"
      return 1
      ;;
    esac
    shift
  ...
  if [[ -z "$input_file" ]]; then
    echo "Error: you have to pass the path of the audio file to transcript with the -f parameter"
    return 1
  fi
...
help() {
  ...
  echo
  echo "audio_file_to_text - Convert an audio file to a text. (it needs a -f parameter specifiyng the path of the audio file to transcript"
  ...
  echo "./smart-actions.sh audio_file_to_text -f /home/file.wav"
}
